...
public class ItemController {
    ...
    public String itemManage(ItemSearchDto itemSearchDto, @PathVariable("page") Optional<Integer> page, Model model) {
        ...
        // PageRequest - Data jpa에서 사용하는 페이지 요청 객체
        // of()메서드를 사용하여 페이지 번호와 페이지당 항목수 지정하여 페이지 요청 정보 생성
        // 0은 rul 경로에서 받아온 페이지 번호를 확인하고, 값이 없으면 0
        // 3은 한 페이지당 보여줄 항목 수


        Page<Item> items = itemService.getAdminItemPage(itemSearchDto, pageable);
        // itemSearchDto를 사영하여 페이지 네이션 된 데이터를 조회
        model.addAttribute("items", items); // 조회된 페이지 네이션된 데이터 모델 추가
        model.addAttribute("itemSearchDto", itemSearchDto); // 검색 조건 모델에 추가
        model.addAttribute("maxPage", 5); // 한 번에 표시할 최대 페이지수를 모델에 추가 // 이 값을 템플리에서 사용하여 페이지 네이션 UI를 그릴때(렌더링) 활용
        return "item/itemMng";
    }

    @GetMapping(value = "/item/{itemId}")
    public String tiemDtl(Model model, @PathVariable("itemId") Long itemId){
        ItemFormDto itemFormDto = itemService.getItemDtl(itemId);
