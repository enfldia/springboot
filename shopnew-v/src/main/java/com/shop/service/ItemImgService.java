...
public class ItemImgService {
    ...
    public void updateItemImg(Long itemImgId, MultipartFile itemImgFile) throws Exception{
        if(!itemImgFile.isEmpty()){
            ...
            // 상품 이미지 아이디를 이용해서 기존에 저장했던 상품 이미지 엔티티를 조회함
    ...
    // 변경된 상품 이미지 정보를 셋팅해 준다.여기서 중요한 점은 상품 등록 때 처럼
    // itemImgRepository.save() 로 로직호출 하지 않는다.
    // savedItemImg 엔티티는 현재 영속 상태 데이터를 변경하는 것만으로 변경 감지 기능이
    // 동작하여 트랜잭션이 끝날 때 update 쿼리가 실행
}
